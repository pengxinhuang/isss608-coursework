...
if(nrow(of_connections) > 0) {
  ...
  of_connections %>%
    arrange(desc(value)) %>%
    head(5) %>%
    mutate(connection = paste(from, "→", to)) %>%
    select(connection, value) %>%
    knitr::kable(col.names = c("Genre Connection", "Number of Artists"))
}
```
...
cat("3. **Career Evolution**: Triple role as lyricist (since 2024), performer (since 2028), and producer\n")
cat("4. **Band Membership**: Part of a 5-member band with Maya Jensen, Lila Hartman, Jade Thompson, and Sophie Ramirez\n")
cat("5. **Work Network**: Connected to 25 other artists through shared songs and albums\n\n")
```
...
cat("1. **Growth Pattern**: Oceanus Folk peaked in", peak_info, "with 34 new releases\n")
cat("2. **Total Reach**: 305 songs spanning 40 years (1992-2040)\n")
cat("3. **Recent Growth**: From 2020 to 2040, the genre grew by", growth_factor, "times\n")
cat("4. **Cross-Genre Influence**: Strong connections with Dream Pop and Indie Folk\n\n")
```

### Rising Stars Predictions

```{r predictions-summary}
cat("Top 5 Predicted Rising Stars (2030-2040):\n")
for(i in 1:min(5, nrow(rising_stars))) {
  star <- rising_stars[i,]
  cat(i, ". **", star$display_name, 
...
1. **For Record Labels**: Focus on artists like Andrew Williams and Brian Wright who show high network connectivity
2. **For Artists**: Follow Sailor Shift's model - combine multiple roles (performer, lyricist, producer) for maximum impact
3. **For Investors**: Monitor Communities 1 and 2 where most rising stars are concentrated
4. **Genre Strategy**: Oceanus Folk artists should consider cross-genre collaborations, especially with Dream Pop
